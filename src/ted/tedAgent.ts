...
export async function runTedAgent(input: TedAgentInput): Promise<{
  reply: string;
  csv?: string;
  upgradeUrl?: string;
...
`;
  ...
  for (let i = 0; i < 8; i++) {
    ...
    for (const call of toolCalls) {
      const toolCall = call as any; // OpenAI SDK types vary between versions
      const name = toolCall.function.name as TedToolName;
      const args = toolCall.function.arguments ? JSON.parse(toolCall.function.arguments) : {};

      const result = await executeTedTool({
        userId: input.userId,
        name,
        rawArgs: args,
      });
  ...
  if (!finalText.trim()) finalText = "Done. What would you like to do next?";
