          ...
          leadSearchId: {
            },
            {
              ...
              function: {
                ...
                description: "Create a new email campaign for outreach. You must specify a name, at least one email account, and either a lead search or list to target. Returns the campaign ID.",
                parameters: {
                  type: "object",
                  properties: {
                    name: { type: "string", description: "Campaign name" },
                    emailAccountIds: { type: "array", items: { type: "string" }, description: "Array of email account IDs to use for sending" },
                    leadSearchId: { type: "string", description: "Lead search ID to target (optional)" },
                    listId: { type: "string", description: "List ID to target (optional)" },
                    steps: {
                      type: "array",
                      items: {
                        type: "object",
                        properties: {
                          ...
                          variantPercent: { type: "number" },
                        },
                        required: ["stepNumber", "delayDays", "subjectLine", "bodyTemplate"]
                      },
                      description: "Array of campaign steps (emails in sequence)"
                    },
                    scheduleStartAt: { type: "string", format: "date-time", description: "When to start sending (optional)" },
                    scheduleEndAt: { type: "string", format: "date-time", description: "When to stop sending (optional)" },
                    sendTimeStart: { type: "string", description: "Daily send window start (e.g. '09:00')" },
                    sendTimeEnd: { type: "string", description: "Daily send window end (e.g. '17:00')" },
                    timezone: { type: "string", description: "Timezone (e.g. 'UTC')" },
                    dailyLimit: { type: "number", description: "Max emails per day" },
                  },
                  required: ["name", "emailAccountIds", "steps"]
                },
              },
            },
            {
              ...
              function: {
                ...
                description: "Import leads from a lead search or list into a campaign and queue emails for outreach. Returns stats on leads and emails queued.",
                parameters: {
                  type: "object",
                  properties: {
                    campaignId: { type: "string", description: "Campaign ID" },
                    minScore: { type: "number", description: "Minimum lead score (optional)" },
                    industry: { type: "string", description: "Industry filter (optional)" },
                    sizeBucket: { type: "string", description: "Company size filter (optional)" },
                    country: { type: "string", description: "Country filter (optional)" },
                    decisionMakerOnly: { type: "boolean", description: "Only decision makers (optional)" },
                    excludePreviousExports: { type: "boolean", description: "Exclude previously exported leads (optional)" },
                    limit: { type: "number", description: "Limit number of leads (optional)" },
                  },
                  required: ["campaignId"]
                },
              },
            },
            {
              ...
              function: {
                ...
                description: "Update the status of a campaign (e.g., start, pause, complete).",
                parameters: {
                  type: "object",
                  properties: {
                    campaignId: { type: "string", description: "Campaign ID" },
                    status: { type: "string", enum: ["DRAFT", "SCHEDULED", "RUNNING", "PAUSED", "COMPLETED", "FAILED"], description: "New status" },
                  },
            ...
            },
            {
              ...
              function: {
                ...
                description: "Get statistics and progress for a campaign (emails sent, opened, replied, etc.).",
                parameters: {
                  type: "object",
                  properties: {
                    campaignId: { type: "string", description: "Campaign ID" },
