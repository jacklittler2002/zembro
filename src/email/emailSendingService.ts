...
import { getSmtpCredentials } from "./emailAccountService";
...
export async function processEmailQueue() {
  ...
  for (const campaign of activeCampaigns) {
    try {
      ...
      logger.error("Error processing campaign queue", {
...
async function processCampaignQueue(campaignId: string) {
  ...
  if (campaign.sendTimeStart && campaign.sendTimeEnd) {
    ...
    if (
      currentTime < campaign.sendTimeStart ||
      currentTime > campaign.sendTimeEnd
    ) {
      logger.info("Outside send time window", {
        campaignId,
        currentTime,
  ...
  if (campaign.dailyLimit && campaign.emailsSentToday >= campaign.dailyLimit) {
    logger.info("Campaign reached daily limit", {
      campaignId,
  ...
  logger.info("Found pending emails", {
    campaignId,
...
export async function sendCampaignEmail(campaignEmailId: string) {
  ...
  if (campaignEmail.status !== "QUEUED") {
    logger.warn("Campaign email is not queued", {
  ...
  if (campaignEmail.emailAccount.status !== "ACTIVE") {
    logger.warn("Email account is not active", {
  ...
  try {
    ...
    const { trackEmailSent } = await import("./emailAccountService");
 ...
 */
export async function trackEmailOpen(campaignEmailId: string) {
  const campaignEmail = await prisma.campaignEmail.findUnique({
    where: { id: campaignEmailId },
  });
 ...
 * Track email reply
 */
export async function trackEmailReply(campaignEmailId: string) {
  const campaignEmail = await prisma.campaignEmail.findUnique({
    where: { id: campaignEmailId },
  });
 ...
 */
export async function trackEmailUnsubscribe(campaignEmailId: string) {
  const campaignEmail = await prisma.campaignEmail.findUnique({
    where: { id: campaignEmailId },
  });
