...
async function handleSiteCrawl(job) {
    if (!job.companyId || !job.targetUrl) {
        logger_js_1.logger.warn("SITE_CRAWL job missing companyId or targetUrl", { jobId: job.id });
        return;
    }
    ...
    if (!company) {
        logger_js_1.logger.warn("SITE_CRAWL job with non-existent company", { companyId: job.companyId });
        return;
    }
    ...
    if (!domain || !(0, domainUtils_js_1.isLikelyBusinessDomain)(domain)) {
        logger_js_1.logger.info("Skipping SITE_CRAWL for non-business domain", {
            domain,
            companyId: company.id,
        });
        return;
    }
    logger_js_1.logger.info("Starting SITE_CRAWL", {
        companyId: company.id,
    ...
    for (const email of emails) {
        await db_js_1.prisma.contact.upsert({
            where: {
                email_companyId: {
                    email,
                    companyId: company.id,
    ...
    logger_js_1.logger.info(`SITE_CRAWL completed for ${company.domain}: pages=${crawlResult.pagesVisited}, emails=${emails.length} (${highQuality.length} high-quality), phones=${phones.length}, score=${score}`);
    logger_js_1.logger.info("Social links found", {
    ...
    if (!existingEnrichment) {
        ...
        logger_js_1.logger.info(`Enqueued ENRICHMENT job for ${company.domain}`);
    }
}
