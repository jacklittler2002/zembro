...
exports.extractAddressGuess = extractAddressGuess;
exports.extractCompanyName = extractCompanyName;
exports.extractBusinessHours = extractBusinessHours;
 ...
 */
function extractAddressGuess(html) {
    const $ = cheerio.load(html);
    ...
    const candidates = lines.filter((line) => {
        ...
        const hasStreetWord = /(street|st\.|road|rd\.|avenue|ave\.|lane|ln\.|way|square|sq\.|park|close|drive|dr\.|court|ct\.|place|pl\.)/i.test(line);
...
function extractCompanyName(html) {
    ...
    if (ogSiteName && ogSiteName.length < 100)
 ...
 */
function extractBusinessHours(html) {
    const $ = cheerio.load(html);
    const text = $("body").text();
...
//# sourceMappingURL=structuredExtractors.js.map