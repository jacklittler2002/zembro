...
const conversationService_js_1 = require("./conversationService.js");
const openaiClient_js_1 = require("../ai/openaiClient.js");
...
async function handleTedChat(userId, userMessage, conversationId) {
    ...
    if (balance < CREDITS_PER_MESSAGE) {
        throw new creditService_js_1.CreditError("INSUFFICIENT_CREDITS", `Insufficient credits. You need ${CREDITS_PER_MESSAGE} credit(s), but have ${balance}.`, {
    ...
    if (!convId) {
        ...
        const conv = await (0, conversationService_js_1.getConversation)(convId);
        if (conv.userId !== userId) {
            throw new Error("Conversation does not belong to this user");
        }
    }
    // Step 3: Append user message
    await (0, conversationService_js_1.appendMessage)(convId, "user", userMessage);
    ...
    await (0, conversationService_js_1.appendMessage)(convId, "assistant", assistantMessage);
    // Step 6: Deduct credits
    await (0, creditService_js_1.consumeCredits)(userId, CREDITS_PER_MESSAGE, "TED chat message", {
        conversationId: convId,
        userMessage: userMessage.substring(0, 50),
    });
    const newBalance = await (0, creditService_js_1.getCreditBalance)(userId);
    logger_js_1.logger.info(`TED chat completed for user ${userId} in conversation ${convId}. Credits used: ${CREDITS_PER_MESSAGE}, remaining: ${newBalance}`);
    return {
        conversationId: convId,
        assistantMessage,
...
async function generateAiResponse(conversationId, userMessage) {
    logger_js_1.logger.info(`Generating AI response for conversation ${conversationId}`);
    ...
    const chatHistory = [
        {
            role: "system",
            content: tools_js_1.TED_SYSTEM_PROMPT,
        },
    ...
    let response = await openaiClient_js_1.openai.chat.completions.create({
        model: "gpt-4o",
        messages: chatHistory,
        tools: tools_js_1.TED_TOOLS,
        tool_choice: "auto",
    });
    ...
    while (response.choices[0]?.finish_reason === "tool_calls" && iterationCount < maxIterations) {
        ...
        logger_js_1.logger.info(`TED making ${toolCalls.length} tool call(s) in iteration ${iterationCount}`);
        ...
        for (const toolCall of toolCalls) {
            ...
            logger_js_1.logger.info(`Executing tool: ${functionName}`, functionArgs);
            // Execute the tool
            const result = await (0, toolExecutor_js_1.executeTool)(conversation.userId, functionName, functionArgs);
        ...
        response = await openaiClient_js_1.openai.chat.completions.create({
            model: "gpt-4o",
            messages: chatHistory,
            tools: tools_js_1.TED_TOOLS,
            tool_choice: "auto",
    ...
    logger_js_1.logger.info(`TED response generated after ${iterationCount} tool call iteration(s)`);
